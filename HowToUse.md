...
├── Data/
│   └── [your-workspace-name]/
│       ├── tasks/            ← "Tasks" icon / Ctrl+T — all your tasks
│       │   └── working/      ← "Working Tasks" pane — tasks you're actively on
│       ├── dailylogs/        ← "Daily Journal" icon / Ctrl+L
...
| **Tasks icon / Ctrl+T** | Opens the full `tasks/` list, grouped by status/priority |
| **Daily Journal icon / Ctrl+L** | Opens/creates today's entry in `dailylogs/` |
| **Search box** | Full-text search across every note (3+ characters) |
| **Ctrl+K** | Command palette — run any command, or search notes (`@`) / tags (`#`) |
| **Ctrl+Space** | Switch between your currently open tabs |
| **+ button** | Create a new note or task in the current folder |
