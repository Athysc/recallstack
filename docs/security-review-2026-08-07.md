...
**Resolved 2026-08-10.** Output cards now assign filenames, subpaths, and title text through DOM properties instead of interpolating workspace names into HTML attributes.
...
**Resolved 2026-08-10.** Primary note paths now reject a symbolic-link `Data` directory and symbolic links in every existing path component, matching the generic native bridge policy.
...
**Resolved 2026-08-10.** Both Mermaid initialization paths now explicitly set `securityLevel: "strict"` instead of inheriting the dependency default.
...
**Resolved 2026-08-11.** The optional CDN language-module path was deleted during desktop cutover. Unknown syntax languages now load only the bundled full highlight.js asset. The former sql.js network/local branch is also gone; Rust SQLite is the only index.
...
|---|---|---|---|
| 1 | `esc()` doesn't escape quotes; used in an HTML attribute → stored XSS via a maliciously named folder (resolved 2026-08-10) | Frontend | **Resolved** |
| 2 | `workspace.rs` note-path validator skips the symlink check `bridge.rs` has (resolved 2026-08-10) | Backend | **Resolved** |
| 3 | Hand-rolled HTML sanitizer (no DOMPurify) — plausible mXSS bypass class | Frontend | Medium-High |
| 4 | Mermaid rendering bypasses the app's sanitizer, relies on unconfigured library default (resolved 2026-08-10) | Frontend | **Resolved** |
