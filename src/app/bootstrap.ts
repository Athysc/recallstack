...
export function prepareRuntimeDependencies(): RuntimeCapabilities {
  ...
  window.__onlineDeps = localStorage.getItem("pkm-online-deps") === "on";
  window.__sqlLocal = capabilities.desktop || localStorage.getItem("pkm-sql-source") === "local";
  window.__depSources = {
    sql: capabilities.desktop ? "native" : window.__sqlLocal ? "local" : "online",
    marked: "local",
    hljs: "local",
    hljsFull: "local",
    mermaid: "local",
