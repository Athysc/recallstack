...
test("keyboard zoom shortcuts move one configured increment and clamp", () => {
  assert.equal(nextContentZoom(0, 1), 10);
  assert.equal(nextContentZoom(50, 1), 60);
  assert.equal(nextContentZoom(50, -1), 40);
  assert.equal(nextContentZoom(100, 1), 100);
  assert.equal(nextContentZoom(0, -1), 0);
});
